...
export const auto_pan_plugin: EaselPlugin = (ctx) => {
  let mouse_x = 0;
  let mouse_y = 0;
  let is_pointer_down = false;
  
  ctx.app_events.on('pointerdown', () => {
    is_pointer_down = true;
  });

  ctx.app_events.on('pointerup', () => {
    is_pointer_down = false;
  });

  ctx.app_events.on('pointermove', (e: PointerEvent) => {
  ...
  const loop = () => {
    if (is_pointer_down) {
      ...
      if (mode === 'dragging' || mode === 'wiring' || mode === 'resizing' || mode === 'box_selecting') {
        const rect = ctx.container.getBoundingClientRect();
        const edge = 40;
        const speed = 12;
        let dx = 0, dy = 0;
        ...
        if (dx !== 0 || dy !== 0) {
          ctx.dispatch(state => {
            ...
            if (inter.mode === 'dragging' || inter.mode === 'resizing' || inter.mode === 'box_selecting') {
              inter = { ...inter, start_pos: vec2_add(inter.start_pos, vec2_create(dx, dy)) } as any;
            }
            
            const next_state = {
              ...state,
              camera: { ...state.camera, position: vec2_add(state.camera.position, vec2_create(dx, dy)) },
              interaction: inter
            };
