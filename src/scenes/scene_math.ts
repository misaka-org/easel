...
export const load_math_scene = (dispatch: Dispatch) => {
  dispatch((s) => add_node(s, {
    ...
    title: 'Number A',
    inputs: [],
    outputs: [{ id: 'out', label: 'Value', type: 'output', value_type: 'number' }],
...
export const evaluate_math_graph = (state: State): State => {
  ...
  for (const node of Object.values(state.nodes)) {
    if (node.type === 'math') {
      const wire_a = Object.values(state.wires).find(w => w.target_node_id === node.id && w.target_port_id === 'a');
      const wire_b = Object.values(state.wires).find(w => w.target_node_id === node.id && w.target_port_id === 'b');
      
      const val_a = wire_a ? get_port_value(wire_a.source_node_id, wire_a.source_port_id) : 0;
      const val_b = wire_b ? get_port_value(wire_b.source_node_id, wire_b.source_port_id) : 0;
      
      let res = 0;
      if (node.custom_data['operation'] === 'add') res = val_a + val_b;
      
      if (node.custom_data['result'] !== res) {
        new_nodes[node.id] = { ...node, custom_data: { ...node.custom_data, result: res } };
        changed = true;
      }
    }
  }
